...
- 후기게시판 :   
  ...
  - 게시물 :   
    - 게시물 쓰기 :   
      게시글 작성 폼을 작성하고 "작성완료" 버튼을 클릭합니다. 폼에 정의된 action 속성에 명시된 경로인 QAWriteSave.php로 HTTP POST 요청을 생성합니다.QAWriteSave.php에서는 $_POST 변수를 사용하여 POST 방식으로 전송된 데이터에 접근할 수 있습니다.   
      사용자가 로그인되어 있는지 확인합니다. 로그인되어 있지 않다면 알림창을 띄웁니다.    
      제목과 내용이 비어있는지 확인하고, 하나라도 비어 있다면 알림창을 띄웁니다.   
      데이터베이스에 데이터를 추가하기 전 real_escape_string 함수를 사용하여 SQL 인젝션을 방지하도록 처리합니다. SQL 쿼리를 사용하여 데이터베이스에 게시글을 추가하고 게시글이 성공적으로 추가되었다는 알림을 띄우고, JavaScript를 사용하여 게시물 목록 페이지로 이동합니다.   
    - 게시물 내용 :    
